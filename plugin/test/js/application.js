var application = new (function() {
	...
	for (i in data.events) {
	    ...
	    console.log("comp: " + item.comp + " type: " + item.type + " id: " + item.id + " status: " + item.status);
	...
	try {
		...
		success: function(data) { application.saveComponentSuccess() },
    ...
    this.saveComponentSuccess = function()
    {
	try {
		...
		var handler = {
		    'success': function() { application.postComponentUpdate(); },
    ...
    this.postComponentUpdate = function() {
	screenManager.unlockLocation();
	// we must reselect, since the objects were recreated
	this.select(this.selectedComponent.id);
	this.componentView.updateComponentView(this.selectedComponent, this.components);
	this.menuView.renderMenuView(this.components);
    };
